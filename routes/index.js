...
module.exports = function(app, passport, Database){
  ...
  app.get('/myprofile', function(req,res){
    ...
    app.get('/checkin',isAuthenticated, function(req, res) {
        res.render('template/checkin', {error:"",now:moment().format('YYYY.MM.DD')});
    });
    ...
    app.post('/checkin/create',upload.single('uploadImage'), function(req, res) {
        ...
        if(point==null||point==""){
          res.render('template/checkin', {error:"Error in Saving checkin",now:moment().format('YYYY.MM.DD')});
          return;
        }
    ...
    function createCheckin(req,res,momentDate,fileId){
      ...
      CheckIn.findOne({ group:req.user.group,date:momentDate, email:req.user.email },function(err,checkIn){
        ...
        if(checkIn ){
          ...
          if(checkIn.fileId){
            var _oldFileId = checkIn.fileId;
            console.log(_oldFileId);


            gfs.remove({_id:_oldFileId}, function (err) {
  ...
  app.get('/latecomer',isAuthenticated, function(req, res) {
     res.render('template/latecomer', {now:moment().format('YYYY.MM.DD'),admin:req.user.admin});
  });
